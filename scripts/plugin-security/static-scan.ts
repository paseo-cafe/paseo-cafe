...
function walk(base: string, dir: string, depth: number, state: { files: number; bytes: number; incomplete: boolean }, findings: SecurityFinding[], buildCommands: string[][], registryId?: string) {
  ...
  for (const entry of readdirSync(dir, { withFileTypes: true })) {
    ...
    if (meta.size > MAX_BYTES) { state.incomplete = true; findings.push(finding("scanner", "size-limit", "high", true, rel, "file exceeds size budget")); continue }
    ...
    if (rel === "paseo-plugin.json") validateManifest(content, rel, registryId, findings, buildCommands)
...
function validateEntrypoint(name: string, path: string, findings: SecurityFinding[]) { if (name === "index.ts") findings.push(finding("entrypoint", "legacy-index", "high", true, path, "legacy-only index.ts is rejected")) }
function scanBoundaries(content: string, path: string, findings: SecurityFinding[]) { if (/from\s+["']\.\.\/(client|server|shared)\//.test(content) || /from\s+["'](?:client|server|shared)\//.test(content)) findings.push(finding("boundary", "cross-runtime-import", "high", true, path, "cross-runtime import boundary violated")) }
