...
export const ImageZoom = ({
  ...
  // react-medium-image-zoom's default export ("Uncontrolled") always manages
  // its own internal zoom state and ignores any `isZoomed` passed to it —
  // there's no way to drive it from outside. We render its `Controlled`
  // variant directly instead, falling back to local state so this still
  // behaves like a plain, uncontrolled zoom when the caller doesn't pass
  // `isZoomed` (e.g. a single image with no gallery navigation).
  const [internalIsZoomed, setInternalIsZoomed] = useState(false)
  const isControlled = isZoomedProp !== undefined

  const handleZoomChange = useCallback<
    NonNullable<ControlledProps["onZoomChange"]>
  >(
    (value, data) => {
      if (!isControlled) setInternalIsZoomed(value)
  ...
  return (
    ...
    >
      <Controlled
        isZoomed={isControlled ? (isZoomedProp as boolean) : internalIsZoomed}
